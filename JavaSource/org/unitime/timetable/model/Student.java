...
public class Student extends BaseStudent implements Comparable<Student>, NameInterface, Qualifiable {
	...
	public static List findAll(Long sessionId) {
		return new StudentDAO().getSession().createQuery("select s from Student s where s.session.uniqueId=:sessionId")
	...
	public static Student findByExternalId(Long sessionId, String externalId) {
		return (Student) new StudentDAO().getSession().createQuery("select s from Student s where " +
				"s.session.uniqueId=:sessionId and " +
				"s.externalUniqueId=:externalId").setLong("sessionId", sessionId.longValue())
				.setString("externalId", externalId).setCacheable(true).uniqueResult();
	}

	public static Student findByExternalIdBringBackEnrollments(org.hibernate.Session hibSession, Long sessionId,
			String externalId) {
		return (Student) hibSession.createQuery("select s from Student s " +
	...
	public int compareTo(Student student) {
		...
		return (getUniqueId() == null ? Long.valueOf(-1) : getUniqueId())
				.compareTo(student.getUniqueId() == null ? -1 : student.getUniqueId());
	}
	...
	public static Hashtable<Long, Set<Long>> findConflictingStudents(Long classId, int startSlot, int length,
		...
		Query q = LocationDAO.getInstance().getSession()
				.createQuery("select distinct e.clazz.uniqueId, e.student.uniqueId " +
						...
						+
						"where x.clazz.uniqueId=:classId and x.student=e.student and " + // only look among students of
																							// the given class
						"e.clazz=c.clazz and " + // link ClassEvent c with StudentClassEnrollment e
						"m.stopPeriod>:startSlot and :endSlot>m.startPeriod and " + // meeting time within given time
																					// period
	...
	public boolean isRequestApproved() {
		return getOverrideStatus() == null
				|| getOverrideStatus().intValue() == CourseRequestOverrideStatus.APPROVED.ordinal();
	}

	public boolean isRequestPending() {
		return getOverrideStatus() != null
				&& getOverrideStatus().intValue() == CourseRequestOverrideStatus.PENDING.ordinal();
	}

	public boolean isRequestCancelled() {
		return getOverrideStatus() != null
				&& getOverrideStatus().intValue() == CourseRequestOverrideStatus.CANCELLED.ordinal();
	}

	public boolean isRequestRejected() {
		return getOverrideStatus() != null
				&& getOverrideStatus().intValue() == CourseRequestOverrideStatus.REJECTED.ordinal();
	}
	...
	public WaitListMode getWaitListMode() {
		...
		if (CustomStudentEnrollmentHolder.isAllowWaitListing()
				&& (status == null || status.hasOption(Option.waitlist))) {
			...
			for (CourseDemand cd : getCourseDemands()) {
				if (getExternalUniqueId() != null && getExternalUniqueId().equals(cd.getChangedBy())) {
					if (ret == null || ret.before(cd.getTimestamp()))
						ret = cd.getTimestamp();
					if (cd.getWaitlistedTimeStamp() != null && (ret == null || ret.before(cd.getWaitlistedTimeStamp())))
		...
		if (getClassEnrollments() != null) {
			for (StudentClassEnrollment e : getClassEnrollments()) {
				if (getExternalUniqueId() != null && getExternalUniqueId().equals(e.getChangedBy())) {
					if (ret == null || ret.before(e.getTimestamp()))
						ret = e.getTimestamp();
				}
			}
		}
	...
	public boolean hasReleasedPin() {
		return getPin() != null && !getPin().isEmpty() && isPinReleased() != null && isPinReleased().booleanValue();
	}

	public String getReleasedPin() {
		return (hasReleasedPin() ? getPin() : null);
	}

	public Set<Long> getAdvisorWaitListedCourseIds(boolean useWaitLists, boolean useNoSubs) {
		if (!useWaitLists && !useNoSubs)
			return null;
		if (getAdvisorCourseRequests() == null || getAdvisorCourseRequests().isEmpty())
			return null;
		Set<Long> courseIds = new HashSet<Long>();
		for (AdvisorCourseRequest acr : getAdvisorCourseRequests()) {
			if (useWaitLists && acr.getWaitlist() != null && acr.getWaitlist().booleanValue()
					&& acr.getCourseOffering() != null) {
				courseIds.add(acr.getCourseOffering().getUniqueId());
			}
			if (useNoSubs && acr.getNoSub() != null && acr.getNoSub().booleanValue()
					&& acr.getCourseOffering() != null) {
				courseIds.add(acr.getCourseOffering().getUniqueId());
			}
		}
	...
	public CourseRequest getCourseRequest(CourseOffering co) {
		for (CourseDemand cd : getCourseDemands())
			for (CourseRequest cr : cd.getCourseRequests())
				if (cr.getCourseOffering().equals(co))
					return cr;
		return null;
	}
	...
	public boolean isEnrolled(CourseOffering co) {
		if (co == null)
			return false;
		for (StudentClassEnrollment e : getClassEnrollments())
			if (e.getCourseOffering().equals(co))
				return true;
		return false;
	}

	public WaitList addWaitList(CourseOffering co, WaitList.WaitListType type, boolean waitListed, String changedBy,
			Date timeStamp, org.hibernate.Session hibSession) {
		...
		if (getWaitlists() != null && !getWaitlists().isEmpty()) {
			for (WaitList wl : getWaitlists()) {
				if ((last == null || last.compareTo(wl) < 0)
						&& (wl.getCourseOffering().equals(co) || wl.hasMatchingCourse(cr)))
					last = wl;
			}
		}
		...
		if (enrolled != null && cr != null && enrolled.equals(cr.getCourseDemand().getWaitListSwapWithCourseOffering())
				&& !cr.isRequired())
		...
		if (lastWaitListed != waitListed || !(lastEnrolled == null ? enrolled == null : lastEnrolled.equals(enrolled))
				|| !(lastSwap == null ? swap == null : lastSwap.equals(swap)) ||
	...
	public void resetWaitLists(WaitList.WaitListType type, String changedBy, Date timeStamp,
		...
		for (StudentClassEnrollment e : getClassEnrollments()) {
			...
			if (wl != null && wl.isWaitListed()) {
				...
				wl.setTimestamp(timeStamp);
				CourseRequest cr = getCourseRequest(e.getCourseOffering());
				wl.setCourseOffering(
						cr != null ? cr.getCourseDemand().getFirstChoiceCourseOffering() : e.getCourseOffering());
				wl.setCourseDemand(cr == null ? null : cr.getCourseDemand());
			...
			if (cd.isWaitlist() && !cd.isEnrolled(true)) {
				...
				if (co != null) {
					...
					if (old == null || !old.isWaitListed() || !WaitList.computeRequest(cd).equals(old.getRequest()) ||
							!(old.getSwapCourseOffering() == null ? cd.getWaitListSwapWithCourseOffering() == null
									: old.getSwapCourseOffering().equals(cd.getWaitListSwapWithCourseOffering()))) {
						// new wait-lists
						WaitList wl = new WaitList();
						wl.setChangedBy(changedBy);
						wl.setTimestamp(timeStamp);
	...
	public ClassModality getPreferredClassModality() {
		if (getSchedulePreference() == null)
			return ClassModality.NoPreference;
	...
	public ModalityPreference getModalityPreference() {
		switch (getPreferredClassModality()) {
			case DiscouragedOnline:
				return ModalityPreference.ONILNE_DISCOURAGED;
			case PreferredOnline:
				return ModalityPreference.ONLINE_PREFERRED;
			case RequiredOnline:
				return ModalityPreference.ONLINE_REQUIRED;
			default:
				return ModalityPreference.NO_PREFERENCE;
		}
	}

	public ScheduleGaps getPreferredScheduleGaps() {
		if (getFreeTimeCategory() == null)
			return ScheduleGaps.NoPreference;
	...
	public BackToBackPreference getBackToBackPreference() {
		switch (getPreferredScheduleGaps()) {
			case DiscourageBackToBack:
				return BackToBackPreference.BTB_DISCOURAGED;
			case PreferBackToBack:
				return BackToBackPreference.BTB_PREFERRED;
			default:
				return BackToBackPreference.NO_PREFERENCE;
		}
	}
}
